...
PYTHON_PATH = (
    r"D:\Spectra\GIS-Real\.venv\Scripts\python.exe"  # Or just "python" if in venv
)
...
class MCPClientManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            nasa_transport = await self.exit_stack.enter_async_context(
                stdio_client(nasa_params)
            )
            self.nasa_session = await self.exit_stack.enter_async_context(
                ClientSession(nasa_transport[0], nasa_transport[1])
            )
            print("   [1/4] NASA Process Launched.")

            print("   [2/4] Launching GIS Process...")
            gis_transport = await self.exit_stack.enter_async_context(
                stdio_client(gis_params)
            )
            self.gis_session = await self.exit_stack.enter_async_context(
                ClientSession(gis_transport[0], gis_transport[1])
            )
...
async def get_nasa_weather_mcp(lat: float, lon: float):
    ...
    try:
        ...
        result = await mcp_manager.call_nasa_tool(
            "nasa_power",
            {
                "latitude": lat,
                "longitude": lon,
...
async def get_gis_data_mcp(location: str):
    try:
        result = await mcp_manager.call_gis_tool(
            "get_coordinates", {"location": location}
        )
        return result.content[0].text
    except Exception as e:
        return {"error": str(e)}
...
async def calculate_ndvi_mcp(lat: float, lon: float):
    ...
    try:
        result = await mcp_manager.call_gis_tool(
            "calculate_ndvi", {"lat": lat, "long": lon}
        )
        return result.content[0].text
    except Exception as e:
        return {"error": str(e)}
